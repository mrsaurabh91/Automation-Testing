...
public class PDFQuestionExtraction extends PDFTextStripper  {
	...
	LinkedHashMap<String,String> linkedHashMapAnswer = new LinkedHashMap<String,String>();
	...
	public void launchBrowser() throws InterruptedException, IOException {
		...
		inputPdfFileName = inputFolderPath+"\\input\\"+pdfFileName;
		answerPdfFileName = inputFolderPath+"\\input\\"+envProperties.getProperty("answerFileName");	;
		...
		try{
			...
			driver.get("file:\\\\\\" + inputPdfFileName);
			Thread.sleep(20000);
...
//						
						...
						//	System.out.println("textInsideBox "+textInsideBox);
							if(!textInsideBox.isEmpty() || !textInsideBox.equalsIgnoreCase("")){
								...
								exactQuestion = removeStartEndTag(questionText);
							
								if(!questionNumber.equalsIgnoreCase("") && !questionText.equalsIgnoreCase("")){
									linkedHashMapQuestion.put(questionNumber.trim(), exactQuestion.trim());
								}
							}
						}else{
							textInsideBox=getRectangleText(x, y, h, w, page,inputPdfFileName).trim();
							if(!textInsideBox.isEmpty() || !textInsideBox.equalsIgnoreCase("")){
								ArrayList<String> optionList = new ArrayList<String>();
								optionList = getAnswerOption(textInsideBox);
								linkedHashMapAnswerOption.put(questionNumber.trim(), optionList);
								questionNumber = "";
							}
						}
					} 
		...
		getCorrectAnswerFromPdf(answerPdfFileName);
		...
		System.out.println("linkedHashMapAnswer "+linkedHashMapAnswer);
		
		if(linkedHashMapQuestion.size() != 0 && linkedHashMapAnswerOption.size() != 0 && linkedHashMapAnswer.size() != 0){
			driver.quit();
			QuestionPourOperation questionPourOperation = new QuestionPourOperation();		
			questionPourOperation.createQuestion(linkedHashMapQuestion,linkedHashMapAnswerOption,linkedHashMapAnswer);
		}else{
			System.out.println("Question, Answer Option or Correct Answer not found");
		}
	...
	public void getCorrectAnswerFromPdf(String answerPdfFileName) throws InterruptedException, IOException{
		
		
		driver.get("file:\\\\\\" +answerPdfFileName);
		Thread.sleep(10000);
		...
		for (int page = 1; page <= 4; page++) {			
			...
			if (blocksCount>0) {
				for (int i = 1; i <= blocksCount; i++) {
					...
					String textInsideBox="";
//					
						textInsideBox=getRectangleText(x, y, h, w, page,answerPdfFileName).trim();
						System.out.println("textInsideBox "+textInsideBox);
						if(!textInsideBox.isEmpty() || !textInsideBox.equalsIgnoreCase("")){
							String[] answerSplitedByNewLine = textInsideBox.split("\n");
							...
							for(int splitedAnswerCount = 0; splitedAnswerCount<answerSplitedByNewLine.length; splitedAnswerCount++){
								...
								if(!questionNumberWithCorrectAnser.isEmpty() || !questionNumberWithCorrectAnser.equalsIgnoreCase(" ")){
									...
									if(!correctAnser.isEmpty() && !questionNumber.isEmpty()){
										linkedHashMapAnswer.put(questionNumber.trim(), correctAnser.trim());
									}
...
/*	public ArrayList<String> getAnswerOption(String str){
	...
	*/
	...
	//	System.out.println("getFontWithChar");
		...
		String fileName = inputPdfFileName;
		try {
	...
	public String getRectangleText(int x, int y, int h, int w, int pageNumber,String fileName) throws IOException {
