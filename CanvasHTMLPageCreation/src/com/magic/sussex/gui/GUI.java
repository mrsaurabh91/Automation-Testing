...
public class GUI extends WebDriverFactory implements ActionListener{
	...
	static JButton hmtlpageCretionSubmitButton;
...
//	JTabbedPane jTabbedPane;  
	...
	*/	
	...
	/*	JLabel sussexLabel = new JLabel("URL");
		sussexLabel.setFont(new Font("verdana", Font.BOLD, 12));
		sussexLabel.setBounds(x, y, 85, 25);
...
*/
		JLabel docSelectLabel = new JLabel("Storyboard Document");
		docSelectLabel.setFont(new Font("verdana", Font.BOLD, 12));
		docSelectLabel.setBounds(x, y += 40, 150, 25);
		...
		hmtlpageCretionSubmitButton = new JButton("Submit");
		hmtlpageCretionSubmitButton.setBounds(x+ 230, y += 40, 80, 25);
		hmtlpageCretionSubmitButton.addActionListener(this);
		htmlCreationPanel.add(hmtlpageCretionSubmitButton);
		...
		JLabel imageUploadUrlLabel = new JLabel("URL");
		imageUploadUrlLabel.setFont(new Font("verdana", Font.BOLD, 12));
		imageUploadUrlLabel.setBounds(xAxis, yAxis, 85, 25);
		...
		JLabel imageUploadUserLabel = new JLabel("Username");
...
/*		if (e.getSource() == imageUploadSubmitButton) {
			...
			if(checkAllInput){
				...
				GUI.logger.info("Execution stops due to lack of required details. Please reset, fill required details and run again");
				GUI.consoleText.append("Execution stops due to lack of required details\n");
				GUI.consoleText.append("Please reset tool, fill required details and run again\n");
			}
		...
		if (e.getSource() == hmtlpageCretionSubmitButton) {
			consoleText.setText("");
			/*url = "";
			userName = "";*/
			courseName = "";
...
*/			docFilePath = "";
			...
			courseName = programTextBox.getText().trim();
	//		folderName = folderNameTextBox.getText().trim();
			...
			/*if(url == "" || url.isEmpty()){
				...
				logger.info("Password is blank");
				 GUI.consoleText.append("Password is blank\n");
				checkAllInput = false;
			}*/if(courseName == "" || courseName.isEmpty()){
			...
			hmtlpageCretionSubmitButton.setEnabled(false);			
		...
		if(e.getSource()==resetBtn){
			...
			hmtlpageCretionSubmitButton.setEnabled(true);	
...
//			password = "";
		...
		if (e.getSource() == tableSubmitBtn) {
			...
			if(checkPageNeedToCreate){
				ExecutionAferPageActionTypeSelected executionAferPageActionTypeSelected = new ExecutionAferPageActionTypeSelected(url, userName, password, courseName, folderName, docFilePath, templateFolderPath);
	...
	public static void setSubmitButton(boolean b) {
		hmtlpageCretionSubmitButton.setEnabled(b);
	}
